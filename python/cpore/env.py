...
SPACE_BONUSES = ("colonise", "trade", "attack")


def _bind_space(lib):
    lib.cp7_env_create.argtypes = [c_uint32]
    lib.cp7_env_create.restype = c_void_p
    lib.cp7_env_free.argtypes = [c_void_p]
    lib.cp7_env_reset.argtypes = [c_void_p, c_uint32, POINTER(c_float), POINTER(c_float)]
    lib.cp7_env_step.argtypes = [c_void_p, POINTER(c_float), POINTER(c_float),
                                 POINTER(c_float), POINTER(c_int32), POINTER(c_int32)]
    lib.cp7_env_obs_dim.restype = c_int32
    lib.cp7_env_act_dim.restype = c_int32
    lib.cp7_env_state_size.restype = c_size_t
    lib.cp7_env_save.argtypes = [c_void_p, c_void_p]
    lib.cp7_env_load.argtypes = [c_void_p, c_void_p]
    lib.cp7_env_world.argtypes = [c_void_p]
    lib.cp7_env_world.restype = c_void_p
    lib.cp7_policy_greedy.argtypes = [c_void_p, POINTER(c_float)]
    lib.cp7_env_census.argtypes = [c_void_p, POINTER(c_int32), POINTER(c_float)]
    lib.cp7_env_status.argtypes = [c_void_p]
    lib.cp7_env_status.restype = c_int32
    return lib


class SpaceEnv:
    """Stage 6. Captain one ship in a galaxy of systems and rival empires:
    settle the free stars, buy loyalties with trade routes, or take them by
    siege - the same fork every scale below, at the scale of a galaxy.

    ``legacy`` is the three multipliers a finished civilisation hands forward
    - colonise, trade, attack - or None for an empire that inherited nothing.
    For the campaign arc::

        civ = CivEnv(seed=7)     # ... play ...
        space = SpaceEnv(seed=7, legacy=[...civ bonus multipliers...])
    """
    ...
    def __init__(self, seed: int = 0, legacy=None, render_size=(1280, 720),
                 vis=DEFAULT_VIS):
        self._lib = _bind_space(lib())
        self.obs_dim = int(self._lib.cp7_env_obs_dim())
        self.act_dim = int(self._lib.cp7_env_act_dim())
        self._h = self._lib.cp7_env_create(c_uint32(seed & 0xFFFFFFFF))
        if not self._h:
            raise MemoryError("cp7_env_create failed")
        self._obs = (c_float * self.obs_dim)()
        self._act = (c_float * self.act_dim)()
        self._rew = c_float()
        self._term = c_int32()
        self._trunc = c_int32()
        self._state_size = int(self._lib.cp7_env_state_size())
        self._cnt = (c_int32 * 7)()
        self._val = (c_float * 8)()
        self.default_legacy = legacy
        self.render_size = render_size
    ...
    def close(self):
        if getattr(self, "_h", None):
            self._lib.cp7_env_free(self._h)
            self._h = None
    ...
    def reset(self, seed=None, legacy=None):
        ...
        if legacy is None:
            ...
            if isinstance(legacy, dict):
                legacy = [legacy.get(n, 1.0) for n in SPACE_BONUSES]
            lp = (c_float * 3)(*[float(v) for v in legacy])
        self._lib.cp7_env_reset(self._h, c_uint32(seed & 0xFFFFFFFF), lp, self._obs)
        return self._out()

    def step(self, action):
        for i in range(self.act_dim):
            self._act[i] = float(action[i])
        self._lib.cp7_env_step(self._h, self._act, self._obs,
                               ctypes.byref(self._rew),
                               ctypes.byref(self._term), ctypes.byref(self._trunc))
        return (self._out(), float(self._rew.value),
                bool(self._term.value), bool(self._trunc.value), {})
    ...
    def greedy_action(self):
        self._lib.cp7_policy_greedy(self._lib.cp7_env_world(self._h), self._act)
        return list(self._act)

    @property
    def status(self) -> str:
        return ("running", "lost", "won", "timeout")[
            int(self._lib.cp7_env_status(self._h))]

    def census(self):
        self._lib.cp7_env_census(self._h, self._cnt, self._val)
        return {"stars": self._cnt[0], "settled": self._cnt[1],
                "flipped": self._cnt[2], "captured": self._cnt[3],
                "lost": self._cnt[4], "trade_runs": self._cnt[5],
                "raids": self._cnt[6],
                "money": self._val[0], "income": self._val[1],
                "tax_earned": self._val[2], "trade_earned": self._val[3],
                "upgrades": self._val[4],
                "bonus": {"colonise": self._val[5], "trade": self._val[6],
                          "attack": self._val[7]}}

    def save_state(self) -> bytes:
        buf = ctypes.create_string_buffer(self._state_size)
        self._lib.cp7_env_save(self._h, buf)
        return buf.raw

    def load_state(self, blob: bytes):
        self._lib.cp7_env_load(self._h, ctypes.create_string_buffer(blob, len(blob)))

    def _draw(self, vis=None):
        w, h = self.render_size
        if self._fb is None:
            self._fb = ctypes.create_string_buffer(w * h * 4)
        self._lib.cp7_render_styled(self._lib.cp7_env_world(self._h), self._fb, w, h, 0)
        return w, h
